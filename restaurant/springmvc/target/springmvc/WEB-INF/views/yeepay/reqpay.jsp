...
<%
	...
	String nodeAuthorizationURL = formatString(Configuration
			.getInstance().getValue("yeepayCommonReqURL")); // 交易请求地址
	// 商家设置用户购买商品的支付信息
	...
	String p6_Pcat = formatString("");
	String p7_Pdesc = formatString("");
	//String p8_Url = formatString(request.getParameter("p8_Url")); // 商户接收支付成功数据的地址
	String p8_Url = formatString("http://localhost:8080/yeepaydemo/yeepayCommon/JAVA/callback.jsp");
	...
	String p9_SAF = formatString("0");
	String pa_MP = formatString("");
	String pd_FrpId = formatString("");
	// 银行编号必须大写
	pd_FrpId = pd_FrpId.toUpperCase();
	String pr_NeedResponse = formatString(request
			.getParameter("pr_NeedResponse")); // 是否需要应答机制
